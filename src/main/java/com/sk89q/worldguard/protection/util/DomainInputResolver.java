    ...
    public DefaultDomain call() throws UnresolvedNamesException {
        ...
        if (!namesToQuery.isEmpty()) {
            try {
                ...
                throw new UnresolvedNamesException("Не удалось получить UUID игроков. Преобразовать ники в UUID невозможно.");
        ...
        if (!namesToQuery.isEmpty()) {
            throw new UnresolvedNamesException("Не удалось получить UUID игрока " + Joiner.on(", ").join(namesToQuery));
        }
